...
private:
    ...
    void ConsiderMixedSums(const std::vector<int>& sumsLeft, const std::vector<int>& sumsRight) {
        for (int sumRight : sumsRight) {
            UpdateResult((sumRight + sumsLeft.back()) % n);
            auto iter = std::lower_bound(sumsLeft.begin(), sumsLeft.end(), n - sumRight); 
    ...
    void UpdateResult(int candidate) {
...
//         std::cin >> nums[i];
//     }
//     std::cout << MaximumSubsequence(m, nums)();
//     return 0;
// }