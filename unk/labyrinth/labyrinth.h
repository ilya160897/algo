...
private:
        ...
        for (int i = 0; i < h; ++i) {
            for (int j = 0; j < w; ++j) {
                ...
                if (IsFree(coord) && CoordToComponent[ID(coord)] == -1) {
                    ...
                    assert(static_cast<int>(ComponentToSize.size()) == CurrentComponent + 1);
                    DFS(coord);
                }
            }
        }
    ...
    bool Valid(const Coord& coord) {
        return 0 <= coord.first && coord.first < h
               && 0 <= coord.second && coord.second < w;
    }
...
// std::string ReadInput(size_t readSize) {
//     const size_t MAX_READ_SIZE = 1010;
//     char Buff[MAX_READ_SIZE];
//     scanf("%s1000007", Buff);

//     std::string result;
//     std::copy(Buff, Buff + readSize, std::back_inserter(result));
//     return result;
// }
...
//     for (int i = 0; i < n; ++i) {
//         table.emplace_back(ReadInput(m));
//     }
//     auto result = Labyrinth(table)();
