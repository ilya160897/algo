...
struct TDayRate {
    ...
    bool operator<(const TDayRate& other) const {
...
struct TResult {
    TResult(Int day, const std::vector<TIdDay>& idDays) : Day(day), IdDays(idDays) {
...
public:
    GadgetsForDollarsAndPounds(size_t days, size_t gtotal, size_t ggoal, Int balance,
                                const std::vector<TDayRate>& drate, const std::vector<TDayRate>& prate,
                                const std::vector<TGadget>& dgadgets, const std::vector<TGadget>& pgadgets) 
        : Days(days) 
        , gTotal(gtotal) 
        , gGoal(ggoal) 
        , Balance(balance) 
        ...
        , dGadgets(dgadgets) 
        , pGadgets(pgadgets) 
        , dGadgetCumsums(dgadgets.size(), 0) 
        , pGadgetCumsums(pgadgets.size(), 0) 
    {
        Init();
    }
